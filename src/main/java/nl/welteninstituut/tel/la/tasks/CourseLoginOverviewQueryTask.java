...
import nl.welteninstituut.tel.la.rest.Query;
 ...
 * ****************************************************************************
 */
public class CourseLoginOverviewQueryTask extends GenericBean {
    private String jobId;

    public CourseLoginOverviewQueryTask() {

    }
    public CourseLoginOverviewQueryTask(String jobId) {
        this.jobId = jobId;
    }
            ...
            if (pollJob.getStatus().getState().equals("DONE")) {
                ...
                if (rows !=null)
                    for (TableRow row : rows) {
                        ...
                        learnerAverageActivities.addRow(courseId, count);
                        Query.bigQuery.courseActivitiesOverview(courseId);
                    }
                QueryCacheManager.addQueryResult("course_login_overview", learnerAverageActivities.toJsonObject().toString());
            } else {
                scheduleTask();
            }
        } catch (IOException e) {
