 ...
 * ****************************************************************************
 */
public class CourseUserDateToObjectDefinitionJob extends Job {
    ...
    public CourseUserDateToObjectDefinitionJob(long date) {
        this.date =date;
    }
...
//        Output<KeyValue<String, UserDateVerb>, Void> output = new CalendarOutput(); //O R
            ...
            try {
                ...
                if (timestampLong > date) {
                    ...
                    if (objectObject.has("objectType") && objectObject.getString("objectType").equals("Activity")){
                        ...
                        String objectDefinition = StringPool.BLANK;
                        try {
                            objectDefinition = objectObject.getJSONObject("definition").getString("type");
                        } catch (Exception e) {

                        }
                        if (objectDefinition != StringPool.BLANK) {
                            UserDateObject userDateVerb = new UserDateObject();
                            userDateVerb.addVerb(objectDefinition);
                            String key = courseId+ ":" + actorId + ":" + format.format(new Date(timestampLong));
                            Entity entityOut = new Entity(KeyFactory.createKey(CourseUserDateToObjectDefinition.TABLE_NAME, key));
                            entityOut.setProperty(CourseUserDateToObjectDefinition.VALUE_COLUMN, new Text(userDateVerb.toJson().toString()));
                            entityOut.setProperty(CourseUserDateToObjectDefinition.ACTORID_COLUMN, actorId);
                            entityOut.setProperty(CourseUserDateToObjectDefinition.COURSEID_COLUMN, courseId);
                            emit(key, entityOut);
                        }
    ...
    class CalendarReducer extends Reducer<String, Entity,  Entity> { //K V O
        ...
        public void reduce(String actorIdDate, ReducerInput<Entity> reducerInput) { //K  ReducerInput<V>
            ...
            resultEntity.setProperty(CourseUserDateToObjectDefinition.VALUE_COLUMN, new Text(resultUserDateVerb.toJson().toString()));
