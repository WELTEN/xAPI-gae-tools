 ...
 * ****************************************************************************
 */
public class CalendarActivitiesJob extends Job {
    ...
    private Long date;
    public CalendarActivitiesJob() {

    }

    public CalendarActivitiesJob(long date) {
        this.date =date;
    }
...
//        Output<KeyValue<String, UserDateVerb>, Void> output = new CalendarOutput(); //O R
        ...
        public void map(Entity entity) {
            try {
                ...
                if (timestampLong > date) {
                    ...
                    String key = actorId + ":" + format.format(new Date(timestampLong));
                    Entity entityOut = new Entity(KeyFactory.createKey(UserDateToVerb.TABLE_NAME, key));
                    entityOut.setProperty(UserDateToVerb.VALUE_COLUMN, new Text(userDateVerb.toJson().toString()));
                    entityOut.setProperty(UserDateToVerb.ACTORID_COLUMN, actorId);
                    emit(key, entityOut);
                } else {
                    LOG.severe("Statement "+entity.getKey().getName()+" is has a date that is too old: "+timestamp);
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    class CalendarReducer extends Reducer<String, Entity,  Entity>{ //K V O
        ...
        public void reduce(String actorIdDate, ReducerInput<Entity> reducerInput) { //K  ReducerInput<V>
            ...
            resultEntity.setProperty(UserDateToVerb.VALUE_COLUMN, new Text(resultUserDateVerb.toJson().toString()));

            emit(resultEntity);
        }
    }

//    class CalendarOutput extends Output<KeyValue<String, UserDateVerb>, Void>{
//
//        @Override
//        public List<? extends OutputWriter<KeyValue<String, UserDateVerb>>> createWriters(int numShards) {
//            ImmutableList.Builder<CalendarWriter> out = ImmutableList.builder();
//            for (int i = 0; i < numShards; i++) {
//                out.add(new CalendarWriter());
//            }
//            return out.build();
//        }
//
//        @Override
//        public Void finish(Collection<? extends OutputWriter<KeyValue<String, UserDateVerb>>> collection) throws IOException {
//            return null;
//        }
//    }
//
//    private class CalendarWriter extends OutputWriter<KeyValue<String, UserDateVerb>> {
//
//        @Override
//        public void write(KeyValue<String, UserDateVerb> o) throws IOException {
//            System.out.println("writing (output) "+o.toString());
//        }
//    }
